...
static void pvr_init_tile_matrix(int which, int presort) {
    ...
    int     x, y, tn;
    b32_uint32      *vr;  /* Note: We're working in 4-byte pointer maths in this function */
    volatile int    *opb_sizes;
    //uint32      matbase, opbbase;

    vr = (b32_uint32*)emu_vram; //(uint32*)PVR_RAM_BASE;
...
void pvr_allocate_buffers(pvr_init_params_t *params) {
    ...
    /* Set screen sizes; pvr_init has ensured that we have a valid mode
       ...
       structure here. */
    pvr_state.w = 640; //vid_mode->width;
    pvr_state.h = 480; //vid_mode->height;
    ...
    pvr_state.pclip_right = 639; //vid_mode->width - 1;
    pvr_state.pclip_top = 0;
    pvr_state.pclip_bottom = 479; //vid_mode->height - 1;
