...
strip tri_stripper::ExtendToStrip(const size_t Start, triangle_order Order)
{
	...
	for (tri_iterator Node = (m_Triangles.begin() + Start); 
		...
		const const_link_iterator Link = LinkToNeighbour(Node, ClockWise, Order, false, 0);

		// Is it the end of the strip?
		if (Link == Node->out_end() || Size >= 42) {
			...
			Node = Link->terminal();
			Indices.push_back(Node - m_Triangles.begin());
	...
	return strip(Start, StartOrder, Size, Indices);
}
...
strip tri_stripper::BackExtendToStrip(size_t Start, triangle_order Order, bool ClockWise)
{
	...
	for (Node = (m_Triangles.begin() + Start); 
		...
		if (Link == Node->out_end() || Size >= 42)
			...
			Node = Link->terminal();
			// Indices.push_back(Node - m_Triangles.begin());
			Indices.insert(Indices.begin(), Node - m_Triangles.begin());
			(* Node)->SetStripID(m_StripID);
			ClockWise = ! ClockWise;
		}
	}
	...
	return strip(Node - m_Triangles.begin(), Order, Size, Indices);
}
...
void tri_stripper::BuildStrip(const strip Strip)
{
	...
	assert(Strip.m_Indices[0] == Start);
	...
	for (size_t Size = 1; Size < Strip.Size(); ++Size) {
		...
		const const_link_iterator Link = LinkToNeighbour(Node, ClockWise, Order, true, Strip.m_Indices[Size]);

		assert(Link != Node->out_end());

		// Go to the next triangle
		Node = Link->terminal();
		assert(Strip.m_Indices[Size] == Node - m_Triangles.begin());
...
inline tri_stripper::const_link_iterator tri_stripper::LinkToNeighbour(const const_tri_iterator Node, const bool ClockWise, triangle_order & Order, const bool NotSimulation, size_t triIndex)
{
...
{
	size_t start_tri = 0;
	while(start_tri != m_Triangles.size()) {
		...
		std::set<size_t> UniqIndices;
		for (size_t i = start_tri; i < m_Triangles.size(); ++i) {
			if (! m_Triangles[i].marked()) {
				Indices.push_back(m_Triangles[i]->A());
				UniqIndices.insert(m_Triangles[i]->A());
				Indices.push_back(m_Triangles[i]->B());
				UniqIndices.insert(m_Triangles[i]->B());
				Indices.push_back(m_Triangles[i]->C());
				UniqIndices.insert(m_Triangles[i]->C());
			}

			start_tri = i + 1;
			if (UniqIndices.size() >= 3) {
				break;
			}
		}
